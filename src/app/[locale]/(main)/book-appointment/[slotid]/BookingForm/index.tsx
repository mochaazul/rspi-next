		...
		try {
			...
			await bookAppointment(payloadBook).then(res => {
				const pushNotifPayload: PayloadPushNotification = {
					...
					text_en: 'Your appointment for ' + selectedProfile?.name?.trim() + ' ' + doctorResponse?.data?.name + ' on ' + timeSlot?.date + ' ' + timeSlot?.session_app_start + ' has been booked. Please check your email for appointment details. If you wish to cancel, please notify us at least 1 hour before the doctors schedule to avoid being blacklisted',
					icon: 'Bell',
					url: '/patient-portal',
					notif_type: '1',
