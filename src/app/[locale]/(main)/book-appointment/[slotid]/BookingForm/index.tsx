		...
		try {
			...
			await bookAppointment(payloadBook).then(res => {
				const pushNotifPayload: PayloadPushNotification = {
					...
					text_idn: 'Permintaan booking anda untuk ' + selectedProfile?.name?.trim() + ' ke ' + doctorResponse?.data?.name + ' pada ' + timeSlot?.date + ' ' + timeSlot?.session_app_start + ' sudah berhasil. Silahkan cek email Anda untuk informasi lebih lanjut. Mohon konfirmasi ketidakhadiran 1 jam sebelum jadwal mulai praktek dokter untuk menghindari blacklist oleh system kami.',
					text_en: "Your appointment for " + selectedProfile?.name?.trim() + " " + doctorResponse?.data?.name + " on " + timeSlot?.date + " " + timeSlot?.session_app_start + " has been booked. Please check your email for appointment details. If you wish to cancel, please notify us at least 1 hour before the doctor's schedule to avoid being blacklisted",
					icon: 'Bell',
					url: '/patient-portal',
					notif_type: '1',
